...
class ChangeRequest(TimeStampedModel):
    ...
    status = models.CharField(max_length=10, choices=CHANGE_STATUS_CHOICES,
                              default=CHANGE_STATUS_CHOICES[0][0], null=True, blank=True)
...
class BaseMember(TimeStampedModel):
    user = models.OneToOneField(User, on_delete=models.SET_NULL, related_name='basemember',
    ...
    clubunit = models.ForeignKey(ClubUnit, on_delete=models.SET_NULL, null=True,
    ...
    physic_information = models.OneToOneField(PhysicInformation, related_name='basemember',
    ...
    #                 self.user.members.all().update(closed=True,
    #                   status=CHANGE_STATUS_CHOICES[1][0])
...
class ArcherMember(BaseMember):
    ...
    approved_by = models.ForeignKey(User, on_delete=models.SET_NULL,
                                    related_name='member_approval', null=True, blank=True)
