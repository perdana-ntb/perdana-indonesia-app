...
class TestMixins:
    ...
    def create_archer_user(self):
        ...
        self.archer_member1 = mommy.make(ArcherMember, user=self.user1)
        ...
        self.archer_member2 = mommy.make(ArcherMember, user=self.user2, approved=True)

    def auth(self, username, password='123'):
        auth_url = reverse('user_api:login')
        data = {'username': username, 'password': password}
        r = self.client.post(auth_url, data)
